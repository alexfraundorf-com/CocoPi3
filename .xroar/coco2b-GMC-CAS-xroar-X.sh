...
then
	...
	startx /usr/local/bin/xroar -c $HOME/.xroar/xroar.conf -default-machine coco2bus -cart gmc -no-cart-autorun -run "$cassette" $XROARPARMS
    ...
    startx /usr/local/bin/xroar -c $HOME/.xroar/xroar.conf -default-machine coco2bus -cart gmc -no-cart-autorun $XROARPARMS
