...
then
	...
	xroar -c $HOME/.xroar/xroar.conf -cart gmc -no-cart-autorun -machine coco2bus -cart mpi -mpi-load-cart 0=gmc -mpi-load-cart 3=rsdos -mpi-slot 3 -load "$floppy"
    ...
    xroar -c $HOME/.xroar/xroar.conf -cart gmc -no-cart-autorun -machine coco2bus -cart mpi -mpi-load-cart 0=gmc -mpi-load-cart 3=rsdos -mpi-slot 3
fi
